...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode,
	...
	//GXP18 - Begin
    	//String aware_msg = getString(R.string.remove_from_settings);
	final ImageView aware_icon = (ImageView)mStatusBarView.findViewById(R.id.awareIcon);
        final TextView aware_secure_msg = (TextView)mStatusBarView.findViewById(R.id.aware);
	aware_secure_msg.setText("Secure Message");
        

        PhoneStatusBar.BroadcastListener mBroadcastListener=new BroadcastListener();
        IntentFilter aware_filter_pic = new IntentFilter("android.intent.action.TAKE_PIC");
	IntentFilter aware_filter_vid = new IntentFilter("android.intent.action.TAKE_VID");
        //filter.addAction(Intent.TAKE_PIC);
        //filter.addAction("TakeVic");
        mContext.registerReceiver(mBroadcastListener, aware_filter_pic); 
        mContext.registerReceiver(mBroadcastListener, aware_filter_vid); 
        //Log.v("CDS84", "CDS84 - BroadcastReceiver Ready");
    ...
    private class BroadcastListener extends BroadcastReceiver {
		...
		//context.unregisterReceiver(mBroadcastListener);
                final TextView aware_secure_msg = (TextView)mStatusBarView.findViewById(R.id.aware);
	      //  Log.v("CDS84", "CDS84 - IntentReceived");
                		
		//android.widget.Toast.makeText(getContext(),"Take Pic Received", 500).show();
            if (intent.getAction().equals("android.intent.action.TAKE_PIC")) {
		aware_secure_msg.setText("Take Picture");
	    
	    } else if (intent.getAction().equals("android.intent.action.MIKE_START")) {
		aware_secure_msg.setText("Record Audio");
        
            } else if (intent.getAction().equals("android.intent.action.TAKE_VID")) {
		aware_secure_msg.setText("Take Video");
	    }
         }
    }
